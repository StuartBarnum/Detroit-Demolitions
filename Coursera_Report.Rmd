...
Given the relative concreteness and understandability of decision trees, together with the fact that these models were only somewhat less effective as predictors than the more advanced models, I suspect that the the decision trees may parovide the best of the models for at least some uses. With this in mind, it is espcially notable that a remarkably simple models may provide a useful predictor. First all all, I run the analysis that provided that most accurate results.
...
models <- 1:10 %>% map(~ rpart(complete_formula, data = complete_tally_set[-k_folds[[.x]],],
                               method = "class", control = rpart.control(cp = 0.003)))

predictions <- 1:10 %>% map(~ predict(models[[.x]], newdata = complete_tally_set[k_folds[[.x]],], type = "class"))
predictions <- 1:10 %>% map(~as.numeric(predictions[[.x]]) - 1)

accuracies <- 1:10 %>% 
  map(~ mean(as.numeric(! predictions[[.x]] < 0.5) == complete_tally_set[k_folds[[.x]],]$blighted))
...
#confusion matrices for the models
for (index in 1:10) {
  print(table(pred = (predictions[[index]] > 0.5), truth = complete_tally_set[k_folds[[index]],]$blighted))
}
...
Adjusting the complexity parameter somewhat higher (increasing the threshold that must be if a split is to be implemented), we abtain the following remarkably simple model, which funcations roughly as the above. 
...
Note that the above decision tree is the result of the rpart model for all of the 10 elements of the partition associated with k-fold cross validation. As is well-known about the city of Detroit, many of of the vacant parcels once contained buildings, which were torn down due to bight. We thus see that past and current nearby blight may provide the best predictors of blight in a particular area. 
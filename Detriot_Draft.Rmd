...
#add row numbers to the set of rows for which there are other rows with the same parcel number
dup_with_row_numbers <- duplicate_parcel_numbers_in_parcel_data %>% mutate(row = row_number())

#join the result with itself, on the basis of sameness of identity of spatial identity 
spatial_repeats <- st_join(dup_with_row_numbers, dup_with_row_numbers, 
                           st_equals, left = FALSE) %>% filter(row.x != row.y) %>%
  mutate(row_num = row_number())

#find a set of the parcels, none of which covers the same area as the any of the others, that convers all of these parcels with spatial repeats.
output <- 1:nrow(spatial_repeats)
for (index in 1:nrow(spatial_repeats)) {
  output[index] <- !(spatial_repeats$row.x[index] %in% spatial_repeats$row.y[1:index]) 
}
output <- as.logical(output)
unique_parcels <- spatial_repeats[as.logical(output),]
unique_parcels

rm(duplicate_parcel_numbers_in_parcel_data, output)
```

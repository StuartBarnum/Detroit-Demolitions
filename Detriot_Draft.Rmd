...
```{r}
#investagate a few of the cases by satelite map, 
area_map <- function(sf_shape) {
...
area_map(parcel_sf %>% filter(parcelnum == "13000116.003"))

```

We begin the process of creating a list of buildings, using on the parcels data. One of the issues to consider is number of buildings on the parcel. Should we restrict our analysis to those parcels that have only one buiding? In any case, two columns in `parcel_sf` bear on this aspect: `building_s` and `num_buildi`.  
```{r}
...
st_coordinates(parcel_sf %>% filter(parcelnum == "13000116.003"))
...
building_5 contains zero rows, thus indicating that all of the rows of buildings_1 is a subset of building_2. 

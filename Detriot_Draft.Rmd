...
as.data.frame(demolition_join_on_parcels) %>%
  select(-geometry.x, -geometry.y, -geometry) %>%
  group_by(parcelnum) %>%
  mutate(n = n()) %>%
  filter(n > 1)
...
* The parcel is a hardest hit area and has at least one building (or perhaps exactly one)
* The parcel is a hardest hit area and reflected in dismantle permits data
* The parcel is a hardest hit area and reflected in the completed demolitions data

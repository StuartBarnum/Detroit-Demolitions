...
The development of the dataset used for modeling, including code and exploratory graphics, can be found at https://stuartbarnum.github.io/Detroit-Demolitions/Detriot_Draft_3.html

The development of the models used for predicting can be found at
https://stuartbarnum.github.io/Detroit-Demolitions/Detroit_models_from_tallies.html.
...
complete_tally_set <- complete_tally_set %>%
  mutate(earlier_nuisance_offences = earlier_nuiscance_offences) %>%
  select(-earlier_nuiscance_offences)
...
#partition the training set into ten subsets, while maintaining a balance 
...
I should note that all of the models generated in the above k-fold cross-validation process have the same form as does the above decision-tree model (with somewhat different break points, such as with 52.5 instead of 50.5 at node 4).  
...
I should emphasize that the above plots represent a subset of the parcels, reduced so as to have a base rate of blightedness (percentage across the dataset) of roughly 50% within the Hardest Hit Areas. (The actual rate of blightedness in these areas is roughly 2%.) Application of this model, or any of the other models discussed in this report, would need to be implemented carefully so as to avoid base rate bias (see https://en.wikipedia.org/wiki/Base_rate_fallacy). Nevertheless, the fact that past building destruction stands out in this way as a predictor of blight should be a useful consideration to policymakers.
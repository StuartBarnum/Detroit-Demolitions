...
distance <- set_units(100, "m")  #for crimes within 100 meters
...
tallies_with_crime <- improve_detroit_and_violations_tallies %>% 
  st_transform(2253) %>% st_buffer(distance)

#ggplot(improve_detroit_and_violations_tallies[100,]) + geom_sf() 

tallies_with_crime <- tallies_with_crime %>%
...
tallies_with_crime <- tallies_with_crime %>% as.data.frame() %>% select(-geometry) %>% 
  group_by(parcelnum) %>% summarize(recorded_crime_incidents = n())
...
formula <- blighted ~ total_fines_by_parcel + number_of_violations_by_parcel + 
  improve_issues_tallies + recorded_crime_incidents
...
tallies_with_building_type <- tallies_with_crime %>% as.data.frame() %>% select(-geometry) %>%
  left_join(parcel_sf %>% as.data.frame() %>% select(parcelnum, building_s), by = "parcelnum")
  
tallies_with_building_type <- tallies_with_building_type %>% 
  left_join(dismantle_permits_sf %>% as.data.frame() %>% select(`Parcel Number`, `Legal Use`),
            by = c("parcelnum" = "Parcel Number"))
...
#temp <- tallies_with_building_type %>% filter(is.na(building_s))
```

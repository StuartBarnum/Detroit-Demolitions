...
We now construct a ballanced set of labels, with roughly equal numbers of blighted and non-blighted examples. We then divide this set into a training set, an initial test set (which can be used for any number of tests), and a final test set, to be withhel for a final test of our final model.
...
Now consider a histogram of the fine totals, with the note that geom_histogram may not be functioning as one would expect.
...
We will next construct a logistic model, using the constructed dataframe that we used for the two plots above.

```{r}

blight_violation_tallies$blighted <- as.factor(blight_violation_tallies$blighted)

model <- glm(blighted ~ total_fines_by_parcel + number_of_violations_by_parcel, 
             data = blight_violation_tallies, family = binomial)
